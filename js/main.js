...
(function () {
  const translations = {
    ...
    b_desc: ["A complete relooking of the Biougnach brand — I redesigned the full visual identity and built the entire marketplace from scratch. It features an AI shopping assistant (Biou) that answers over live inventory, a fully translated interface in six languages, separate dashboards for clients, vendors and admins, Google authentication, and a checkout with Stripe (test mode) and cash-on-delivery — plus flash sales, wishlists and multi-currency support.", "Un relooking complet de la marque Biougnach — j'ai repensé toute l'identité visuelle et construit l'intégralité de la marketplace. Elle intègre un assistant d'achat IA (Biou) qui répond sur l'inventaire en direct, une interface entièrement traduite en six langues, des tableaux de bord séparés pour clients, vendeurs et administrateurs, une authentification via Google, et un système de paiement Stripe (mode test) ainsi que le paiement à la livraison — avec ventes flash, listes de souhaits et support multi-devises."],
    ...
    c_desc: ["A healthcare booking platform with role-based workflows for patients and admins. Full doctor management with photos, bios and star ratings, an interactive scheduling engine with real-time status tracking, and a production deployment on Render using Docker, with a managed Aiven database, custom Nginx configuration and automated HTTPS.", "Une plateforme de réservation médicale avec des workflows par rôle pour patients et administrateurs. Gestion complète des médecins avec photos, biographies et notes, un moteur de planification interactif avec suivi de statut en temps réel, et un déploiement en production sur Render avec Docker, base de données managée Aiven, configuration Nginx personnalisée et HTTPS automatique."],
